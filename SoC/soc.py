...
class TotalNet(nn.Module):
    ...
    def forward(self, x):
        if hasattr(self.feature_extractor, "extract_features"):
            ...
            logits = self.feature_extractor.classifier(features)
        else:
            logits = self.feature_extractor(x)
            features = None  # Feature extraction is not defined

        return logits, features

class SoC:
    def __init__(self, net_builder, num_classes, ema_m, lambda_cos,\
                 it=0, num_eval_iter=1000, num_train_iter=1000, num_tracked_batch=None, alpha=None, tb_log=None, logger=None, gpu=None, save_dir=None, save_name=None, multiprocessing_distributed=False, rank=0):
        ...
        self.train_model = self.train_model.to(device)
        self.eval_model = self.eval_model.to(device)    
    ...
    def _eval_model_update(self):
        ...
        for param_train, param_eval in zip(self.train_model.parameters(), self.eval_model.parameters()):
            param_eval = param_eval.to(param_train.device)
    ...
    def train(self, logger=None):
        ...
        C = round(self.num_classes / self.alpha) + 1
        C_lower = 1

        label_matrix = torch.tensor(self.label_matrix) if self.soc_resume else torch.zeros(self.num_classes,self.num_classes,self.N)
        label_count = 0
        label_bank = {}
        label_dics = [{} for i in range(C)]
        clusters = [[] for i in range(C)]
        centroids = self.centroids if self.soc_resume else [np.random.choice([i for i in range(self.num_classes)],i+C_lower, False).tolist() for i in range(C)] 

        c_flag = True
        for (x_lb, y_lb), (x_ulb, idx) in zip(self.loader_dict['train_lb'], self.loader_dict['train_ulb']):                
            y_lb = y_lb.long() - 1
            # x_ulb_w = data[0]
            # x_ulb_s = data[1]
            # idx = data[3]
            ...
            x_lb, x_ulb = x_lb.float().cuda(self.gpu), x_ulb.float().cuda(self.gpu)
            ...
            print(f"pseudo_label shape: {pseudo_label.shape}")
            # perform CTT
            for i in range(len(idx)):
                if not idx[i].cpu().item() in label_bank.keys():
                    ...
                    if label_bank[idx[i].cpu().item()] != max_idx[i].cpu().item():
                        label_matrix[label_bank[idx[i].cpu().item()],max_idx[i].cpu().item(),label_count] += 1                            
                        label_bank[idx[i].cpu().item()] = max_idx[i].cpu().item()     
            label_count = (label_count + 1) % self.N
            label_matrix[:,:,label_count] = torch.zeros(self.num_classes,self.num_classes)
            ...
            if self.it % self.num_eval_iter == 0 and c_flag:
                c_flag = False
                for i in range(C):
                    label_dics[i], clusters[i], centroids[i] = kmeans(torch.sum(label_matrix, axis=2, keepdim=False).numpy(), i+C_lower, centroids[i])
                    print(f"label_dics[{i}]: {label_dics[i]}")
                    print(f"clusters[{i}]: {clusters[i]}")      
                    print(f"centroids[{i}]: {centroids[i]}")
                    print(f"Expected clusters (k): {min(i+C_lower, C)}")
            c_count = self.it % C       
            label_dics[c_count], clusters[c_count], centroids[c_count] = kmeans(torch.sum(label_matrix, axis=2, keepdim=False).numpy(), c_count+C_lower, centroids[c_count])              
            for i, dic in enumerate(label_dics):
                print(f"label_dics[{i}]: {dic}")

            del logits

            sup_loss = ce_loss(logits_x_lb, y_lb, reduction='mean')
            cos_loss = consistency_loss_soc(logits_x_ulb, label_dics, clusters, self.alpha, self.num_classes)
            ...
            if self.it % self.num_eval_iter == 0:                
                eval_dict = self.evaluate(lb_loader=self.loader_dict['train_lb'],ulb_loader=self.loader_dict['eval_ulb'],label_dic=label_dics,cluster=clusters)
            ...
            if not self.multiprocessing_distributed or \
                    (self.multiprocessing_distributed and self.rank % ngpus_per_node == 0):
            ...
            del tb_dict
            start_batch.record()
            if self.it == 0.5 * self.num_train_iter:
    ...
    @torch.no_grad()
    def evaluate(self, eval_loader=None, lb_loader=None, ulb_loader=None, label_dic=None, cluster=None):
        ...
        acc_cluster = 0.0
        totalnum = 0.0
        for data in ulb_loader:
            image, target, idx = data
            
            image = image.type(torch.FloatTensor).cuda()
            num_batch = image.shape[0]
            print(num_batch)
            print(f"Image shape before feature extractor: {image.shape}")
            logits = feature_extractor(image)
             
            pseudo_label = torch.softmax(logits, dim=-1)
            max_probs, max_idx = torch.max(pseudo_label, dim=-1)  
            print("Shape of pseudo_label:", pseudo_label.shape)
            ...
            for idx, p in enumerate(pseudo_label):
                ...
                conf_idx = round((max_probs_p.cpu().item())*10)
                print(f"max_probs_p: {max_probs_p.cpu().item()}, conf_idx: {conf_idx}, cluster length: {len(cluster)}")
                if conf_idx >= len(cluster):    
                    print(f"Skipping: conf_idx ({conf_idx}) is out of bounds for cluster of length {len(cluster)}")
                    continue
                if len(cluster[conf_idx]) == 0:
                    label_dic[conf_idx] = None
                       
                if conf_idx not in label_dic: 
                    print(f"Skipping: conf_idx ({conf_idx}) not found in label_dic or is None")
                    continue
                if label_dic[conf_idx] is None:  # If None is assigned, skip further processing
                    continue
                if max_idx[idx].cpu().item() >= len(label_dic[conf_idx]):
                    print(f"Skipping: max_idx[idx] ({max_idx[idx].cpu().item()}) out of bounds for label_dic[conf_idx]")
                    continue
